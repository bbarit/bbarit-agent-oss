...
/// CommonJS/dynamic module loads the AST walk misses: `require('x')` and
/// `import('x')` appear at arbitrary nesting depth (assignments, spreads,
/// function bodies), so a source scan is more reliable than enumerating
/// tree-sitter shapes.
static JS_REQUIRE_RE: once_cell::sync::Lazy<regex::Regex> = once_cell::sync::Lazy::new(|| {
    regex::Regex::new(r#"\b(?:require|import)\s*\(\s*['"]([^'"\n]+)['"]\s*\)"#).unwrap()
});
...
fn extract_imports(source: &str, language: &str, root: &Node) -> Vec<String> {
    ...
    for child in root.children(&mut cursor) {
        match language {
            ...
            "javascript" | "typescript" => {
                // `import x from 'y'` and re-exports (`export { a } from 'y'`,
                // `export * from 'y'`) both carry a `source` field.
    ...
    if matches!(language, "javascript" | "typescript") {
        for cap in JS_REQUIRE_RE.captures_iter(source) {
            imports.push(cap[1].to_string());
        }
        let mut seen = HashSet::new();
        imports.retain(|imp| seen.insert(imp.clone()));
    }
...
fn resolve_import(
    ...
    // Relative paths: ./foo, ../foo. Join + normalize lexically with forward
    // slashes — Path::join would produce `\` on Windows and never match the
    // `/`-separated index paths, and naive `.`-stripping corrupts `../x` and
    // dotted filenames like `./file.min`.
    if raw_import.starts_with('.') {
        let source_dir = Path::new(source_file)
            ...
            .to_string_lossy()
            .replace('\\', "/");
        let mut segments: Vec<&str> = source_dir.split('/').filter(|s| !s.is_empty()).collect();
        for seg in raw_import.split(['/', '\\']) {
            match seg {
                "" | "." => {}
                ".." => {
                    segments.pop();
        ...
        let base_str = segments.join("/");

        for path in all_paths {
            let slashed = path.replace('\\', "/");
            let without_ext = Path::new(&slashed).with_extension("");
            if without_ext.to_string_lossy().replace('\\', "/") == base_str {
                return Some(path.clone());
            }
            // Directory import resolving to index.js/index.ts
            if slashed.starts_with(&format!("{base_str}/"))
                && Path::new(&slashed)
                    .file_stem()
                    .map(|s| s == "index")
                    .unwrap_or(false)
...
mod tests {
    ...
    #[test]
    fn test_commonjs_requires_and_reexports_resolve() {
        let index_src = r#"'use strict';
module.exports = {
  ...require('./calc'),
  ...require('./str'),
};
const lazy = () => import('./queue');
export { daysBetween } from './date';
"#;
        let mut graph = DependencyGraph::new();
        graph.add_file("src/index.js", index_src, "javascript");
        graph.add_file("src/calc.js", "function sum() {}\n", "javascript");
        graph.add_file("src/str.js", "function reverse() {}\n", "javascript");
        graph.add_file("src/queue.js", "class BoundedQueue {}\n", "javascript");
        graph.add_file("src/date.js", "function daysBetween() {}\n", "javascript");
        let node = graph.files.get("src/index.js").unwrap();
        for dep in ["./calc", "./str", "./queue", "./date"] {
            assert!(
                node.raw_imports.iter().any(|imp| imp == dep),
                "missing {dep} in {:?}",
                node.raw_imports
            );
        }
        // Duplicates from AST + regex passes are collapsed.
        let unique: HashSet<_> = node.raw_imports.iter().collect();
        assert_eq!(unique.len(), node.raw_imports.len());
        graph.resolve_dependencies();
        let node = graph.files.get("src/index.js").unwrap();
        assert!(
            node.depends_on.iter().any(|d| d.contains("calc")),
            "resolved: {:?}",
            node.depends_on
        );
        assert!(!graph.dependents("src/calc.js").is_empty());
    }
