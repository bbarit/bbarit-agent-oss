...
fn openai_responses(call: ProviderCall) -> Result<Completion> {
    ...
    if config.stream {
        ...
        return Ok(finalize_stream_outcome(
            parse_openai_responses_sse(std::io::BufReader::new(response))?,
            "OpenAI Responses",
        ));
    }
...
fn openai_completions(call: ProviderCall) -> Result<Completion> {
    ...
    if config.stream {
        ...
        let response = send_with_retry(request, config, body)?;
        return Ok(finalize_stream_outcome(
            parse_openai_sse(std::io::BufReader::new(response), tools)?,
            "OpenAI Chat",
        ));
    }
    let response: Value = send_json_value(request, config, body)?;
...
fn anthropic_messages(call: ProviderCall) -> Result<Completion> {
    ...
    if config.stream {
        ...
        loop {
            ...
            match parse_anthropic_sse(std::io::BufReader::new(response), tools, is_oauth) {
                Err(error)
...
fn google_generate_content(
    ...
    if config.stream {
        ...
        return Ok(finalize_stream_outcome(
            parse_gemini_sse(std::io::BufReader::new(response))?,
            "Gemini",
        ));
    }
