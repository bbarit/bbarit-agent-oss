...
fn read_config(config: &AppConfig) -> HashMap<String, ServerConfig> {
    ...
    for path in [
        ...
        for (name, def) in servers {
            // Only stdio servers (a `command`) are supported in this minimal client.
            ...
            let env = def
                ...
                .unwrap_or_default();
            // cwd entry wins; only insert if not already present.
            out.entry(name.clone()).or_insert(ServerConfig {
